---
name: routing
description: This skill should be used when the user wants to "route requests", "classify input and delegate", "dynamic decision-making", "conditional agent flow", "dispatch to sub-agents", "intent classification", "triage incoming requests", or build a system that must choose between multiple specialized tools, agents, or workflows based on context. Also responds to Korean: "요청 라우팅", "입력 분류 후 위임", "동적 의사결정 에이전트", "조건부 에이전트 흐름", "인텐트 분류", "요청 트리아지". Apply this skill to design or implement the Routing agentic design pattern.
version: 1.0.0
---
