...
description: This skill should be used when the user wants to build "GUI agent", "computer use agent", "browser automation agent", "desktop automation", "visual agent", "screen interaction agent", "web automation agent", "agent computer interaction", "ACI agent", "click automation", "multimodal agent with vision", "Project Mariner style agent", or agents that perceive and interact with graphical user interfaces, web browsers, or physical-world environments. Also responds to Korean: "GUI 에이전트", "컴퓨터 사용 에이전트", "브라우저 자동화 에이전트", "데스크톱 자동화", "시각 에이전트", "화면 상호작용 에이전트". Apply this skill to design or implement agents that interact with computers and real-world environments beyond APIs.
version: 1.0.0
---
