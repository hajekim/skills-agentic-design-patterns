...
description: This skill should be used when the user wants to "agents that improve over time", "self-improving agents", "reinforcement learning", "few-shot learning", "online learning", "agent fine-tuning", "adapt from experience", "learn from feedback", "update agent behavior", "self-modifying agents", "RLHF", "DPO", or build agents that evolve their capabilities based on new data, feedback, or interactions. Also responds to Korean: "시간에 따라 개선되는 에이전트", "자기 개선 에이전트", "강화 학습 에이전트", "경험에서 학습", "피드백 기반 적응", "에이전트 파인튜닝". Apply this skill to design or implement the Learning and Adaptation agentic design pattern.
version: 1.0.0
---
