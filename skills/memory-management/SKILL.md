...
description: This skill should be used when the user wants to "persist agent state", "remember past conversations", "short-term memory", "long-term memory", "session management", "vector database for agents", "agent memory", "context window management", "store user preferences", "recall previous interactions", "stateful agents", or build agents that retain and utilize information across multiple turns or sessions. Also responds to Korean: "에이전트 상태 영속성", "이전 대화 기억", "단기 장기 기억 관리", "세션 관리", "사용자 선호도 저장", "상태 유지 에이전트". Apply this skill to design or implement the Memory Management agentic design pattern.
version: 1.0.0
---
