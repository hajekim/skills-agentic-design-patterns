...
description: This skill should be used when the user wants to build agents with "Google AgentSpace", "no-code agent builder", "agent designer Google", "AI Applications Google Cloud", "Google Cloud no-code agent", "enterprise agent platform", "no-code AI agent", "AgentSpace prompt gallery", "Google Agent Designer", "Knowledge Graph agent", "Google AI Applications", or deploy agents within an organization without writing code. Also responds to Korean: "Google AgentSpace", "노코드 에이전트 빌더", "에이전트 디자이너", "엔터프라이즈 에이전트 플랫폼", "코드 없이 에이전트 배포", "Google AI Applications". Apply this skill to design or implement no-code agent deployments using Google AgentSpace's Agent Designer.
version: 1.0.0
---
